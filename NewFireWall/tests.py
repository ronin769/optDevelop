...
def searchservice(request):
    ...
    if serviceName:
        serviceobj = serviceobj.filter(serviceName__icontains=serviceName)
    if serviceProtocol:
        serviceobj = serviceobj.filter(serviceProtocol__icontains=serviceProtocol)
    if serviceComment:
        serviceobj = serviceobj.filter(serviceComment__icontains=serviceComment)
...
def searchaddress(request):
    ...
    if addressName:
        addressobj = addressobj.filter(addressName__icontains=addressName)
    if addressIP:
        addressobj = addressobj.filter(addressIP__icontains=addressIP)
    if addressComment:
        addressobj = addressobj.filter(addressComment__icontains=addressComment)
...
def searchpolicy(request):
    ...
    if ruleSa:
        policyobj = policyobj.filter(ruleSa__icontains=ruleSa)
    if ruleDa:
        policyobj = policyobj.filter(ruleDa__icontains=ruleDa)
    if ruleService:
        policyobj = policyobj.filter(ruleService__icontains=ruleService)
    if ruleComment:
        policyobj = policyobj.filter(ruleComment__icontains=ruleComment)
...
def getsourceip(request):
    ...
    if "any" in ruleSa:
        ...
        addressobj = AddressName.objects.filter(addressName__icontains=ruleSa.replace(" ", ""))
        ipaddr = ''
        for address in addressobj:
        ...
        for source in sourceip:
            addressobj = AddressName.objects.filter(addressName__icontains=source.replace(" ", ""))
            ipaddress = ''
            for address in addressobj:
...
def getdirectionip(request):
    ...
    if "" == ruleDa:
        ...
        for direction in directionip:
            addressobj = AddressName.objects.filter(addressName__icontains=direction.replace(" ", ""))
            ipaddress = ''
            for address in addressobj:
        ...
        addressobj = AddressName.objects.filter(addressName__icontains=ruleDa.replace(" ", ""))
        ipaddr = ''
        for address in addressobj:
...
def getruleservice(request):
    ...
    if "any" == ruleService:
        ...
        for rule in ruleservice:
            # print("rule:", rule)
            # serviceobj = ServiceName.objects.filter(serviceName__icontains="rule")
            serviceobj = ServiceName.objects.filter(serviceName__icontains=rule.replace(" ", ""))
            serv = ''
        ...
        serviceobj = ServiceName.objects.filter(serviceName__icontains=ruleService.replace(" ", ""))
        service = ''
